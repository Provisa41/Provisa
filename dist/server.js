...
const publicDir = process.env.VERCEL === "1"
...
export function createServer(bot) {
    ...
    app.get("/api/news", (req, res) => {
        const countryId = typeof req.query.country === "string" ? req.query.country : undefined;
    ...
    app.post("/api/consult/request", async (req, res) => {
        ...
        try {
            const data = validateInitData(initData, config.botToken);
            const topic = typeof req.body?.topic === "string" ? req.body.topic.slice(0, 500) : "";
            const phone = typeof req.body?.phone === "string" ? req.body.phone.slice(0, 32) : undefined;
