import { countriesIntroText, documentsText, welcomeText, } from "./copy.js";
import { formatNewsDigestHtml, getNews } from "../data/visaData.js";
import { countriesListKeyboard, countryDetailKeyboard, getCountryDetailText, } from "./countries.js";
import { mainReplyKeyboard, sectionInlineKeyboard, welcomeInlineKeyboard, } from "./keyboards.js";
...
export function registerBotHandlers(bot) {
    ...
    bot.command("start", async (ctx) => {
        ...
        if (startParam) {
            await ctx.reply(`Параметр deep link: <code>${startParam}</code>`, { parse_mode: "HTML" });
        }
    });
    ...
    bot.hears("🗺 Страны", (ctx) => ctx.reply(countriesIntroText, {
        parse_mode: "HTML",
        reply_markup: countriesListKeyboard(),
    }));
    bot.hears("🌍 Обновления", (ctx) => ctx.reply(formatNewsDigestHtml(5), {
        parse_mode: "HTML",
        reply_markup: sectionInlineKeyboard("updates"),
    }));
    ...
    bot.on("message", async (ctx, next) => {
        ...
        if (known.includes(ctx.message.text ?? ""))
            return next();
        await ctx.reply("Команды: /countries /documents /updates /consult — или кнопки меню.", { reply_markup: mainReplyKeyboard() });
    });
}
