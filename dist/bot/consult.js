...
const pendingConsultUsers = new Set();
...
export async function promptConsult(ctx) {
    if (!ctx.from)
        return;
    pendingConsultUsers.add(ctx.from.id);
    await ctx.reply(`👤 <b>Консультация</b>
...
export function registerConsultHandlers(bot) {
    ...
    bot.on("message:contact", async (ctx, next) => {
        if (!ctx.from || !pendingConsultUsers.has(ctx.from.id))
            return next();
        pendingPhones.set(ctx.from.id, ctx.message.contact.phone_number);
        await ctx.reply("Номер сохранён. Теперь отправьте текстом: страна, тип визы, сроки.", { reply_markup: consultContactKeyboard() });
    });
    bot.on("message:text", async (ctx, next) => {
        ...
        if (!pendingConsultUsers.has(ctx.from.id))
            return next();
        pendingConsultUsers.delete(ctx.from.id);
        ...
        await ctx.reply(delivered
            ...
            : "✅ Заявка принята. Мы свяжемся с вами в ближайшее время.", { reply_markup: { remove_keyboard: true } });
    });
}
